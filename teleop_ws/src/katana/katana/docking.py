...
class DockingController(Node):
    def __init__(self):
        ...
        self.ALIGNMENT_STEADY_WINDOW = 0.5 # seconds to pause between alignment turns
        ...
        # Search and alignment state tracking
        ...
        # Publishers
        self.auto_vel_pub = self.create_publisher(Float32, 'auto_velocity', self.qos)
        self.auto_steering_pub = self.create_publisher(Float32, 'auto_steering', self.qos)
        ...
        self.SEARCHING_DOCK = 'searching_dock'
        self.ALIGNING_DOCK = 'aligning_dock'
        self.APPROACHING_DOCK = 'approaching_dock'
        self.SLOW_APPROACH_DOCK = 'slow_approach_dock'
        self.BACKING_UP = 'backing_up'
        self.SEARCHING_HOME = 'searching_home'
        self.ALIGNING_HOME = 'aligning_home'
        self.APPROACHING_HOME = 'approaching_home'
        self.SLOW_APPROACH_HOME = 'slow_approach_home'
        self.FINISHED = 'finished'
        
        self.current_state = self.SEARCHING_DOCK
    ...
    def action_completed(self):
        ...
        if self.current_action == 'search_rotate':
            ...
            return elapsed >= self.BACKUP_WINDOW
        return True
    ...
    def publish_commands(self, velocity, steering):
        ...
        self.auto_vel_pub.publish(vel_msg)
        
        steering_msg = Float32()
        steering_msg.data = steering
        self.auto_steering_pub.publish(steering_msg)
    ...
    def check_state_transition(self):
        ...
        if not self.auto_mode or self.manual_mode or self.ebrake_active:
            self.publish_commands(0.0, 0.0)
            return
            
        # State machine logic
        if self.current_state == self.SEARCHING_DOCK:
            if self.current_tag_id == 0 and not self.tag_lost():
                self.set_state(self.ALIGNING_DOCK)
                self.alignment_is_turning = True
                self.start_action('align_turn')
            else:
                self.handle_search()
                
        elif self.current_state == self.ALIGNING_DOCK:
            if self.current_tag_id != 0 or self.tag_lost():
                self.set_state(self.SEARCHING_DOCK)
                self.search_is_rotating = True
                self.start_action('search_rotate')
            else:
                ...
                if turn_dir == 'center':  # Tag is in center 20%
                    if self.tag_distance <= self.SLOW_APPROACH_DISTANCE:
                        self.set_state(self.SLOW_APPROACH_DOCK)
                        self.start_action('slow_forward')
                    else:
                        self.set_state(self.APPROACHING_DOCK)
                        self.start_action('forward')
                else:
                    self.handle_alignment(turn_dir)
                    
        elif self.current_state == self.APPROACHING_DOCK:
            if self.current_tag_id != 0 or self.tag_lost():
                self.set_state(self.SEARCHING_DOCK)
                self.search_is_rotating = True
                self.start_action('search_rotate')
        ...
        elif self.current_state == self.SLOW_APPROACH_DOCK:
            if self.current_tag_id != 0 or self.tag_lost():
                self.set_state(self.SEARCHING_DOCK)
                self.search_is_rotating = True
                self.start_action('search_rotate')
            elif self.tag_distance is not None and self.tag_distance <= self.DOCKING_DISTANCE:
                self.set_state(self.BACKING_UP)
                self.start_action('backup')
            else:
                if self.action_completed():
                    if not self.is_centered():
                        self.set_state(self.ALIGNING_DOCK)
        ...
        elif self.current_state == self.BACKING_UP:
            if self.action_completed():
                self.set_state(self.SEARCHING_HOME)
                self.search_is_rotating = True
                self.start_action('search_rotate')
            else:
                self.publish_commands(-self.FORWARD_SPEED, 0.0)
                
        elif self.current_state == self.SEARCHING_HOME:
            if self.current_tag_id == 1 and not self.tag_lost():
                self.set_state(self.ALIGNING_HOME)
                self.alignment_is_turning = True
                self.start_action('align_turn')
            else:
                self.handle_search()
                
        elif self.current_state == self.ALIGNING_HOME:
            if self.current_tag_id != 1 or self.tag_lost():
                ...
                if turn_dir == 'center':  # Tag is in center 20%
                    if self.tag_distance <= self.SLOW_APPROACH_DISTANCE:
                        self.set_state(self.SLOW_APPROACH_HOME)
                        self.start_action('slow_forward')
                    else:
                        self.set_state(self.APPROACHING_HOME)
                        self.start_action('forward')
                else:
                    self.handle_alignment(turn_dir)
                    
        elif self.current_state == self.APPROACHING_HOME:
            if self.current_tag_id != 1 or self.tag_lost():
                self.set_state(self.SEARCHING_HOME)
                self.search_is_rotating = True
                self.start_action('search_rotate')
        ...
        elif self.current_state == self.SLOW_APPROACH_HOME:
            if self.current_tag_id != 1 or self.tag_lost():
                self.set_state(self.SEARCHING_HOME)
                self.search_is_rotating = True
                self.start_action('search_rotate')
