...
def plot_step_response_with_params(times, velocities, K, L, T, pwm, pid_params, steady_velocity):
    ...
    fig = plt.figure(figsize=(15, 12))  # Made taller for additional text
    ...
    notes_text = (
        ...
        "• Lower values = Slower, more stable response\n"
        "• Use when simplicity is needed and steady-state error is acceptable\n\n"
        "PI Control (Proportional + Integral):\n"
        "• Higher Ki = Faster error correction but more oscillation\n"
        "• Lower Ki = Slower but more stable error correction\n"
        "• Use when steady-state error must be eliminated\n\n"
        "PID Control (Proportional + Integral + Derivative):\n"
        "• Higher Kd = More damping, less overshoot\n"
        "• Lower Kd = Less damping, faster response\n"
        "• Use when both fast response AND stability are required"
    )
    
    plt.figtext(0.1, 0.02, notes_text, fontsize=10,
