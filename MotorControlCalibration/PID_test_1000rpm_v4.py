...
CONFIG = {
    ...
    'pid_params': {
        ...
        'pi': {'Kp': 48.00, 'Ki': 214.00, 'alpha': 0.4}, 
        'pid': {'Kp': 20.0, 'Ki': 900.0, 'Kd': .50, 'alpha': 0.3}  
    }
}
...
class MotorController:
    def __init__(self, port='/dev/ttyUSB0', baudrate=115200):
        ...
        self.last_time = None
        self.reading_count = 0  # Add reading counter
    ...
    def get_velocity(self, current_position, current_time=None):
        ...
        if self.last_position is None:
            ...
            self.last_time = current_time
            self.reading_count = 0  # Reset reading counter
            return 0.0
        ...
        if dt >= CONFIG['sample_time']:
            ...
            readings_to_skip = 2  # We're always dealing with high PWM in PID
            if self.reading_count <= readings_to_skip:
                self.last_position = current_position
                self.last_time = current_time
                return 0.0
...
def run_pid_test(pid_type='pid'):
    ...
    # Add explicit zero point at start
    times.append(0)
    velocities.append(0)
...
def plot_results(results, save_dir):
    ...
    main_title = f"PID Step Response Test V4 - 1000RPM)\n"
